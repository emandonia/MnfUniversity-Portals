    ...
    public static class URLBuilder
    {
        ...
        public static string GetURLIfExistsforNewsImage(Page page, SiteFolders folder, string filename,int id,string abbr, string language = null)
        {
            ...
            var dc = new PortalDataContextDataContext();
            {
                ...
                {
                    ownerId = (Guid)Prtl_NewsUtility.Get_media_NewsByID(id).Owner_ID;
                }
                else if (abbr.ToLower() == "dent")
                {
                    ownerId = (Guid)Prtl_NewsUtility.Get_dent_NewsByID(id).Owner_ID;
                }
        ...
        public static string UniNewItemUrl(RouteData routeData, int id,string abbr,string ViewerId)
        {
            ...
            {
               type =Prtl_OwnersUtility. GetTypeByAbbr(abbr);
            }
            //else if (abbr.ToLower() == "fci")
            //{
            //    if (Prtl_NewsUtility.Get_univ_NewsByID(id) != null)
            ...
            else if (ViewerId == "NewsViewerControl1")
            {
                ...
                {
                    var x = routeData;
                        var z=CurrentFacAbbr(routeData);
                    var y = CurrentOwnerAbbr(routeData);
                   // return URL2("~", routeData, CurrentFacAbbr(routeData) + "/" + CurrentOwnerAbbr(routeData) + "/NewsDetails/" + id, StaticUtilities.Currentlanguage(routeData));

                    return URL2("~", routeData,CurrentFacAbbr(routeData)+ "/NewsDetails/" + id, StaticUtilities.Currentlanguage(routeData));
                }
                else
                    return URL2("~", routeData,
        ...
        public static string NewItemUrl(RouteData routeData, int id,string abbr)
        {
            ...
            var dc = new PortalDataContextDataContext();
            {
                ...
                {
                    var guid = Prtl_NewsUtility.Get_media_NewsByID(id);
                    if (guid != null)
                        ownerId = (Guid)guid.Owner_ID;
                }
                //else if (abbr.ToLower() == "dent")
        ...
        {
            string x = CurrentFacAbbr(routeData) + "" + abbr + "" + currentlanguage;
        ...
        //  return URL3("~", routeData, CurrentOwnerAbbr(routeData) + "/NewsDetails/" + id, StaticUtilities.Currentlanguage(routeData));

        public static string URL3(string root, RouteData routeData,string abbr, string currentlanguage = null)
        {
            if (currentlanguage == null)
            {
                currentlanguage = StaticUtilities.Currentlanguage(routeData);
        ...
        public static string URLFormat3(string ownerpath, string fac,string lang)
        {
            return (fac+"/"+ownerpath + "/") + lang;
        }
