...
public static class StaticUtilities
{
    ...
    public static IEnumerable<Prtl_MenuUtility.MenuItem> GetMenuItems(string currentLanguage, string position, Guid? ownerID = null)
    {
        ...
        {
            return from mt in dc.prtl_menu_media_trans
                   orderby mt.prtl_menu_media.Parent_id, mt.prtl_menu_media.Order
                   where mt.prtl_Language.LCID == currentLanguage && mt.prtl_menu_media.Published && mt.prtl_menu_media.Position == position
                   && (mt.prtl_menu_media.Owner_ID.GetValueOrDefault(Guid.Empty) == ownerID.GetValueOrDefault(Guid.Empty)
                   || mt.prtl_menu_media.Owner_ID == null)
                   select new Prtl_MenuUtility.MenuItem
                   {
                       Parent_id = mt.prtl_menu_media.Parent_id,
                       Translation_Data = mt.Translation_Data,
                       Url = mt.prtl_menu_media.Url,
                       Id = mt.prtl_menu_media.Menu_id,

                       Url_target = mt.prtl_menu_media.Url_target,

                       Roles = mt.prtl_menu_media.Roles
                   };
        }
        //fatma6-6-2022
        else if (abbr != null && (abbr.ToLower() == "dent" && type == 1))
        {
            return from mt in dc.prtl_menu_dent_trans
                   orderby mt.prtl_menu_dent.Parent_id, mt.prtl_menu_dent.Order
                   where mt.prtl_Language.LCID == currentLanguage && mt.prtl_menu_dent.Published && mt.prtl_menu_dent.Position == position
                   && (mt.prtl_menu_dent.Owner_ID.GetValueOrDefault(Guid.Empty) == ownerID.GetValueOrDefault(Guid.Empty)
                   || mt.prtl_menu_dent.Owner_ID == null)
                   select new Prtl_MenuUtility.MenuItem
                   {
                       Parent_id = mt.prtl_menu_dent.Parent_id,
                       Translation_Data = mt.Translation_Data,
                       Url = mt.prtl_menu_dent.Url,
                       Id = mt.prtl_menu_dent.Menu_id,

                       Url_target = mt.prtl_menu_dent.Url_target,

                       Roles = mt.prtl_menu_dent.Roles
                   };
        }
        else
        {
    ...
    static XElement GetMenuXElement(Prtl_MenuUtility.XMLMenuItem item, Page currentPage, string position)
    {
        ...
        element.SetAttributeValue("TitleFr", item.TitleFr + "");
        element.SetAttributeValue("TitleJa", item.TitleJa + "");
        element.SetAttributeValue("TitleDe", item.TitleDe + "");

        element.SetAttributeValue("TitleTr", item.TitleTr + "");
        element.SetAttributeValue("TitleFa", item.TitleFa + "");
        element.SetAttributeValue("TitleZh", item.TitleZh + "");
        element.SetAttributeValue("TitleRu", item.TitleRu + "");
        ...
        element.SetAttributeValue("UrlFr", URLBuilder.GetURL(item, currentPage, "fr") + "");
        element.SetAttributeValue("UrlJa", URLBuilder.GetURL(item, currentPage, "ja") + "");
        element.SetAttributeValue("UrlDe", URLBuilder.GetURL(item, currentPage, "de") + "");

         element.SetAttributeValue("UrlTr", URLBuilder.GetURL(item, currentPage, "tr") + "");
        element.SetAttributeValue("UrlFa", URLBuilder.GetURL(item, currentPage, "fa") + "");
        element.SetAttributeValue("UrlZh", URLBuilder.GetURL(item, currentPage, "zh") + "");
        element.SetAttributeValue("UrlRu", URLBuilder.GetURL(item, currentPage, "ru") + "");
