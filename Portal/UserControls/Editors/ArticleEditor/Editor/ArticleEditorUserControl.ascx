<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ArticleEditorUserControl.ascx.cs"
    Inherits="MnfUniversity_Portals.UserControls.Editors.ArticleEditor.ArticleEditorUserControl" %>
<%@ Import Namespace="App_Code" %>
<%@ Import Namespace="Common" %>
<div style="overflow: auto; width: 100%;">
    <asp:ListView ID="ArticlesListView" runat="server" DataKeyNames="ID" DataSourceID="ArticlesLinqDataSource"
            ...
            <tr>
                <td colspan="6">
                    <asp:LinkButton ValidationGroup="InsertValidation" OnClick="ArticleEditorControlInsertClicked"
                        ...
                        <asp:Label ID="Label1" runat="server" Text="Insert New Article"></asp:Label>
                    </asp:LinkButton>
                </td>
            </tr>
        </InsertItemTemplate>
        <ItemTemplate>
            <tr runat="server" id="ListViewDataRow" align="center">
                ...
                <td>
                    <asp:Label ID="title_Label" Text='<%# ArticleTitle(Eval("ID"))  %>' runat="server" />
                </td>

                <td>
                    <asp:Label ID="MenuItemLbl" Text='<%# GetMenuItemText(Eval("ID")) %>' runat="server" />
                </td>
                <td class="DirLTR">
                ...
                <td>
                    <asp:HyperLink ID="HyperLink1" NavigateUrl='<%#StaticUtilities.GetArticleItemURL(Page, Convert.ToInt32(Eval("ID")))%>' Text='<%#StaticUtilities.GetArticleItemURL(Page, Convert.ToInt32(Eval("ID")))%>' runat="server"></asp:HyperLink>
                        ...
                        <table id="itemPlaceholderContainer" runat="server" border="1" class="ListViewLayoutTable">
                            <tr id="Tr2" runat="server" class="ListViewLayoutTableHeader">
                                ...
                                <th id="Th8" runat="server">
                                    <asp:Label ID="MenuItemLbll" runat="server" Text="MenuItem"></asp:Label>
                                </th>
    ...
    <asp:LinqDataSource ID="ArticlesLinqDataSource" runat="server" ContextTypeName="Portal_DAL.PortalDataContextDataContext"
        EnableDelete="True" EntityTypeName="" TableName="prtl_Articles" Where="Owner_ID == Guid?(@ID)"
        OrderBy="ID desc">
        <WhereParameters>
            <asp:SessionParameter SessionField="ArticleOwner_ID" Name="ID" DbType="Guid" />
        </WhereParameters>
    </asp:LinqDataSource>
    <uc:ArticleDetailsViewControl ID="ArticleDetailsViewControl1" runat="server" OnUpdateSource="DetailsViewControl_UpdateSource"
