<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="EditForm4.aspx.cs" Inherits="AnnualYouthWeekWebApplication.UI.EditForm4" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContentPlaceHolder" runat="server"><div style="width: 100%">
       <asp:DetailsView ID="DetailsView1" runat="server" style="width: 50%"  AutoGenerateRows="False" DataSourceID="LinqDataSource1">
         <Fields>
             <asp:TemplateField HeaderText="رقم الاستمارة">
                                <EditItemTemplate>
                                    <asp:TextBox ID="txtname3331" runat="server" Text='<%# Eval("SerialNo") %>'></asp:TextBox><asp:Label ID="Label3000" runat="server" Text="يجب ان يكون الرقم من 32 إلي 39 فقط"></asp:Label>
                ...
                <asp:Label ID="Label231" Text='<%# Eval("SerialNo") %>' runat="server" />
                            ...
                            <asp:TemplateField HeaderText="الاسم">
                                <EditItemTemplate>
                                    <asp:TextBox ID="txtname1" runat="server" Text='<%# Bind("Comp_Name") %>'></asp:TextBox>
                                ...
                                <ItemTemplate>
                <asp:Label ID="Label1" Text='<%# Bind("Comp_Name") %>' runat="server" />
            </ItemTemplate>
                            </asp:TemplateField>
             ...
             <asp:TemplateField HeaderText="النوع">
                ...
                <asp:Label ID="Label333" Text='<%# Bind("Email") %>' runat="server" />
                            ...
                            <asp:TemplateField HeaderText="نوع المرافق">
                                <EditItemTemplate>
                                    <asp:DropDownList ID="DropDownList1" SelectedValue='<%#CompanionsUtilty.getadmintype(Convert.ToInt32(Eval("ID"))) %>'  AppendDataBoundItems="True" runat="server" DataSourceID="LinqDataSource3" DataTextField="Comp_Type" DataValueField="ID"></asp:DropDownList>
                                    <asp:LinqDataSource runat="server" EntityTypeName="" ID="LinqDataSource3" ContextTypeName="AnnualYouthWeekWebApplication.MyDataContext" Select="new (Comp_Type, ID)" TableName="Companion_types"></asp:LinqDataSource>



                                </EditItemTemplate>
                                 <ItemTemplate>
                <asp:Label ID="Label32423" Text='<%# GetadminType(Eval("Comp_type")) %>' runat="server" />
            </ItemTemplate>
                            </asp:TemplateField>
    ...
    <asp:LinqDataSource runat="server" EntityTypeName="" ID="LinqDataSource1" ContextTypeName="AnnualYouthWeekWebApplication.MyDataContext"  Where="ID == @ID" Select="new (SerialNo,ID,Comp_Name, Comp_type, National_ID, Gender, Birth_Date, Birth_Place, NatID_Image, Personal_Image, Address, Email, phone_no)" TableName="Companions">
        <WhereParameters>
