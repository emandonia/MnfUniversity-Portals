...
namespace MisBLL
{
    public class Staff_Utility
    {
        ...
        public static void getStfEmail(string nationalNumber, out string email, out string password)
        {
            ...
            var query = (from x in dc.GS_CONTACT_METHOD_Ds
                         where x.GS_CONTACT_METHOD_H_ID == staffmember.GS_CONTACT_METHOD_H_ID && staffmember.STF_NATIONAL_ID_NUM == nationalNumber
                      && x.METHOD_DESCR.EndsWith("menofia.edu.eg")
            ...
            if (currentLang == "ar")
            {
                List<decimal> MemberIDs =
                    (from s in dc.SA_SC_QUALs where s.AS_CODE_DEGREE_ID == degreeID select (s.SA_STF_MEMBER_ID)).Take(100).ToList
                        <decimal>();
                var x = (from s in dc.SA_STF_MEMBERs
                         where MemberIDs.Contains(s.SA_STF_MEMBER_ID) && s.STF_FULL_NAME_AR.Contains(Name) && (s.SA_CODE_JOB_STATUS_ID == 1 || s.SA_CODE_JOB_STATUS_ID == 3 || s.SA_CODE_JOB_STATUS_ID == 8 || s.SA_CODE_JOB_STATUS_ID == 10 || s.SA_CODE_JOB_STATUS_ID == 11)
                         select new { Name = s.STF_CL_FULL_NAME_AR, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_AR, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_AR, Image = "" }).Take(100);
            ...
            else
            {
                List<decimal> MemberIDs =
                    (from s in dc.SA_SC_QUALs where s.AS_CODE_DEGREE_ID == degreeID select (s.SA_STF_MEMBER_ID)).Take(100).ToList
                        <decimal>();
                var x = (from s in dc.SA_STF_MEMBERs
                         where MemberIDs.Contains(s.SA_STF_MEMBER_ID) && s.STF_FULL_NAME_EN.Contains(Name) && (s.SA_CODE_JOB_STATUS_ID == 1 || s.SA_CODE_JOB_STATUS_ID == 3 || s.SA_CODE_JOB_STATUS_ID == 8 || s.SA_CODE_JOB_STATUS_ID == 10 || s.SA_CODE_JOB_STATUS_ID == 11)
                         select new { Name = s.STF_FULL_NAME_EN, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_EN, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_EN, Image = "" }).Take(100);
        ...
        public static object GetMembersByDepAndName(decimal DeptNodeID, string currentLang, string Name, Page page)
        {
            ...
            if (currentLang == "ar")
            {
                var x = (from s in dc.SA_STF_MEMBERs where s.AS_NODE_ID == Convert.ToDecimal(DeptNodeID) && s.STF_FULL_NAME_AR.Contains(Name) && (s.SA_CODE_JOB_STATUS_ID == 1 || s.SA_CODE_JOB_STATUS_ID == 3 || s.SA_CODE_JOB_STATUS_ID == 8 || s.SA_CODE_JOB_STATUS_ID == 10 || s.SA_CODE_JOB_STATUS_ID == 11) select new { ID = s.SA_STF_MEMBER_ID, Name = s.STF_FULL_NAME_AR, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_AR, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_AR, Job = s.SA_CODE_SC_DEG.SC_DEG_DESCR_AR, Job2 = s.SA_CODE_JOB_STATUS.JOB_STATUS_DESCR });

                return x;
            }
            else
            {
                var x = (from s in dc.SA_STF_MEMBERs where s.AS_NODE_ID == Convert.ToDecimal(DeptNodeID) && s.STF_FULL_NAME_EN.Contains(Name) && (s.SA_CODE_JOB_STATUS_ID == 1 || s.SA_CODE_JOB_STATUS_ID == 3 || s.SA_CODE_JOB_STATUS_ID == 8 || s.SA_CODE_JOB_STATUS_ID == 10 || s.SA_CODE_JOB_STATUS_ID == 11) select new { ID = s.SA_STF_MEMBER_ID, Name = s.STF_FULL_NAME_EN, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_EN, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_EN, Job = s.SA_CODE_SC_DEG.SC_DEG_DESCR_EN, Job2 = s.SA_CODE_JOB_STATUS.JOB_STATUS_DESCR_EN });
        ...
        public static object GetMembersByFac(decimal FacID, string currentLang, Page page)
        {
            ...
            if (currentLang == "ar")
            {
                ...
                // var Degrees = (from s in dc.SA_STF_MEMBERs where s.AS_FACULTY_INFO_ID == FacID && (s.SA_CODE_JOB_STATUS_ID != 5 || s.SA_CODE_JOB_STATUS_ID != 7) select new { ID = s.SA_STF_MEMBER_ID, Name = s.STF_FULL_NAME_AR, EnName = s.STF_FULL_NAME_EN, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_AR, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_AR, Job = s.SA_CODE_SC_DEG.SC_DEG_DESCR_AR, Job2 = s.SA_CODE_JOB_STATUS.JOB_STATUS_DESCR });
                var Degrees = (from s in dc.SA_STF_MEMBERs where s.AS_FACULTY_INFO_ID == FacID && (s.SA_CODE_JOB_STATUS_ID == 1 || s.SA_CODE_JOB_STATUS_ID == 3 || s.SA_CODE_JOB_STATUS_ID == 8 || s.SA_CODE_JOB_STATUS_ID == 10 || s.SA_CODE_JOB_STATUS_ID == 11) select new { ID = s.SA_STF_MEMBER_ID, Name = s.STF_FULL_NAME_AR, EnName = s.STF_FULL_NAME_EN, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_AR, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_AR, Job = s.SA_CODE_SC_DEG.SC_DEG_DESCR_AR, Job2 = s.SA_CODE_JOB_STATUS.JOB_STATUS_DESCR });

                return Degrees;
            }
            else
            {
                //var Degrees = (from s in dc.SA_STF_MEMBERs where s.AS_FACULTY_INFO_ID == FacID && (s.SA_CODE_JOB_STATUS_ID != 5 || s.SA_CODE_JOB_STATUS_ID != 7) select new { ID = s.SA_STF_MEMBER_ID, Name = s.STF_FULL_NAME_EN, EnName = s.STF_FULL_NAME_EN, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_EN, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_EN, Job = s.SA_CODE_SC_DEG.SC_DEG_DESCR_EN, Job2 = s.SA_CODE_JOB_STATUS.JOB_STATUS_DESCR_EN });
                var Degrees = (from s in dc.SA_STF_MEMBERs where s.AS_FACULTY_INFO_ID == FacID && (s.SA_CODE_JOB_STATUS_ID == 1 || s.SA_CODE_JOB_STATUS_ID == 3 || s.SA_CODE_JOB_STATUS_ID == 8 || s.SA_CODE_JOB_STATUS_ID == 10 || s.SA_CODE_JOB_STATUS_ID == 11) select new { ID = s.SA_STF_MEMBER_ID, Name = s.STF_FULL_NAME_EN, EnName = s.STF_FULL_NAME_EN, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_EN, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_EN, Job = s.SA_CODE_SC_DEG.SC_DEG_DESCR_EN, Job2 = s.SA_CODE_JOB_STATUS.JOB_STATUS_DESCR_EN });
        ...
        public static object GetMembersByFac2(decimal FacID, Page page)
        {
            ...
            //   var Degrees = (from s in dc.SA_STF_MEMBERs where s.AS_FACULTY_INFO_ID == FacID && (s.SA_CODE_JOB_STATUS_ID != 5 || s.SA_CODE_JOB_STATUS_ID != 7) orderby s.STF_FULL_NAME_AR select s );
            var Degrees = (from s in dc.SA_STF_MEMBERs where s.AS_FACULTY_INFO_ID == FacID && (s.SA_CODE_JOB_STATUS_ID == 1 || s.SA_CODE_JOB_STATUS_ID == 3 || s.SA_CODE_JOB_STATUS_ID == 8 || s.SA_CODE_JOB_STATUS_ID == 10 || s.SA_CODE_JOB_STATUS_ID == 11) orderby s.STF_FULL_NAME_AR select s);
        ...
        public static object GetMembersByFacAndName(decimal FacID, string currentLang, string Name, Page page)
        {
            ...
            if (currentLang == "ar")
            {
                ...
                var Degrees = (from s in dc.SA_STF_MEMBERs where s.AS_FACULTY_INFO_ID == FacID && s.STF_FULL_NAME_AR.Contains(Name) && (s.SA_CODE_JOB_STATUS_ID == 1) select new { ID = s.SA_STF_MEMBER_ID, Name = s.STF_FULL_NAME_AR, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_AR, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_AR, Job = s.SA_CODE_SC_DEG.SC_DEG_DESCR_AR, Job2 = s.SA_CODE_JOB_STATUS.JOB_STATUS_DESCR });

                return Degrees;
            }
            else
            {
                var Degrees = (from s in dc.SA_STF_MEMBERs where s.AS_FACULTY_INFO_ID == FacID && s.STF_FULL_NAME_EN.Contains(Name) && (s.SA_CODE_JOB_STATUS_ID == 1) select new { ID = s.SA_STF_MEMBER_ID, Name = s.STF_FULL_NAME_EN, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_EN, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_EN, Job = s.SA_CODE_SC_DEG.SC_DEG_DESCR_EN, Job2 = s.SA_CODE_JOB_STATUS.JOB_STATUS_DESCR_EN });
        ...
        public static object GetMembersByName(string currentLang, string Name, Page page)
        {
            ...
            if (currentLang == "ar")
            {
                var x = (from s in dc.SA_STF_MEMBERs where s.STF_FULL_NAME_AR.Contains(Name) && (s.SA_CODE_JOB_STATUS_ID == 1) select new { ID = s.SA_STF_MEMBER_ID, Name = s.STF_FULL_NAME_AR, Abbr = getMemberAbbr(s.SA_STF_MEMBER_ID, page), DepName = s.AS_NODE.NODE_DESCR_AR, FacName = s.AS_FACULTY_INFO.FACULTY_DESCR_AR, Job = s.SA_CODE_SC_DEG.SC_DEG_DESCR_AR, Job2 = s.SA_CODE_JOB_STATUS.JOB_STATUS_DESCR });
        ...
        {
            var dc = Global.M_dc;
            var query = (from x in dc.SA_STF_MEMBERs where x.STF_NATIONAL_ID_NUM == NID.Trim() && (x.SA_CODE_JOB_STATUS_ID == 1 || x.SA_CODE_JOB_STATUS_ID == 3 || x.SA_CODE_JOB_STATUS_ID == 8 || x.SA_CODE_JOB_STATUS_ID == 10 || x.SA_CODE_JOB_STATUS_ID == 11) select x).SingleOrDefault();
            if (query != null)
            {
                return query;
            }
        ...
        public static IEnumerable<SA_SC_RESEARCH> GetReasearchesBySTFID(decimal? staffid)
        {
            using (var dc = new MisDataContext())
            {
                var reserachIds = dc.SA_RESEARCH_TEAMs.Where(r => r.SA_STF_MEMBER_ID == staffid).Select(r => r.SA_SC_RESEARCH_ID);
                var researches = dc.SA_SC_RESEARCHes.Where(r => reserachIds.Contains( r.SA_SC_RESEARCH_ID)).ToArray();
                return researches;
