...
public partial class LinkBrowserPage : Page
{
    ...
    protected void SelectImage(object sender, EventArgs e)
    {
        ...
        string type = (link.EndsWith(".swf") ? "src" : "url");
        OkButton.OnClientClick = "window.top.opener.CKEDITOR.dialog.getCurrent().setValueOf('info', '"+type+"', encodeURI('" + link + "')); window.top.close(); window.top.opener.focus();";
    }
    ...
    {
        get { return Session[Request.Params["CKEditor"] + "_FilesFolder"].ToString(); }
    }
    ...
    protected string FileLinkFolderRoot
    {
        get
        {
            var path = URLBuilder.PhysicalPath(LinkFolderRoot);
