...
namespace CKEditor.NET
{
	...
	public class CustomEditor : TextBox, IPostBackDataHandler
	{
		...
		public string BasePath
		{
			get
			{
				...
				if (obj == null)
				{
					...
					ViewState["BasePath"] = (obj ?? "~/ckeditor");
				}
		...
		public CKEditorConfig config
		{
			get
			{
				if (ViewState["CKEditorConfig"] == null)
					ViewState["CKEditorConfig"] = new CKEditorConfig(BasePath.StartsWith("~") ? ResolveUrl(BasePath) : BasePath);
		...
		public override Unit Height
		{
			get
			{
				var result = new Unit(string.Empty);
				try
				{
...
even if the aligment style is not explicitly applied to the current paragraph in the editor.")]
		...
		private static Type _typScriptManager;
		private static Type _updatePanel;
		private static System.Reflection.MethodInfo _mtdRegisterClientScriptInclude;
		private static System.Reflection.MethodInfo _mtdRegisterStartupScript;
		private static System.Reflection.MethodInfo _mtdRegisterOnSubmitStatement;

		private void RegisterClientScriptInclude(Type type, string key, string url)
		{
			if (HasMsAjax)
			{
				if (_mtdRegisterClientScriptInclude == null)
					_mtdRegisterClientScriptInclude = _typScriptManager.GetMethod("RegisterClientScriptInclude", new Type[4] { typeof(Control), typeof(Type), typeof(string), typeof(string) });
				_mtdRegisterClientScriptInclude.Invoke(null, new object[4] { this, type, key, url });
			}
			else
		...
		private void RegisterStartupScript(Type type, string key, string script, bool addScriptTags)
		{
			if (HasMsAjax)
			{
				if (_mtdRegisterStartupScript == null)
					_mtdRegisterStartupScript = _typScriptManager.GetMethod("RegisterStartupScript", new Type[5] { typeof(Control), typeof(Type), typeof(string), typeof(string), typeof(bool) });
				_mtdRegisterStartupScript.Invoke(null, new object[5] { this, type, key, script, addScriptTags });
			}
			else
		...
		private void RegisterOnSubmitStatement(Type type, string key, string script)
		{
			if (HasMsAjax)
			{
				if (_mtdRegisterOnSubmitStatement == null)
					_mtdRegisterOnSubmitStatement = _typScriptManager.GetMethod("RegisterOnSubmitStatement", new Type[4] { typeof(Control), typeof(Type), typeof(string), typeof(string) });
				_mtdRegisterOnSubmitStatement.Invoke(null, new object[4] { this, type, key, script });
			}
			else
				...
				if (_hasMsAjax == null)
				{
					...
					foreach (System.Reflection.Assembly asm in AppAssemblies)
					{
						if (asm.ManifestModule.Name == "System.Web.Extensions.dll")
						{
							try
							{
								_updatePanel = asm.GetType("System.Web.UI.UpdatePanel");
								_typScriptManager = asm.GetType("System.Web.UI.ScriptManager");
								if (_typScriptManager != null) _hasMsAjax = true;
							}
							catch { }
							break;
						}
		...
		{
			base.OnPreRender(e);
			RegisterStartupScript(GetType(), "CKEDITOR_BASEPATH", string.Format("window.CKEDITOR_BASEPATH = '{0}/';\n", (CKEditorJSFile.StartsWith("~") ? ResolveUrl(BasePath) : BasePath)), true);
			RegisterStartupScript(GetType(), "ckeditor", "<script src=\"" + (CKEditorJSFile.StartsWith("~") ? ResolveUrl(CKEditorJSFile) : CKEditorJSFile) + timestamp + "\" type=\"text/javascript\"></script>", false);
			string scriptInit = string.Empty;
			string doPostBackScript = string.Empty;
			if (AutoPostBack) doPostBackScript = string.Format(@"CKEDITOR.instances['{0}'].on('blur', function() {{if(checkDirty()){{javascript:setTimeout('__doPostBack(\'{0}\',\'\')',0); }}}});", ClientID);
			...
			scriptInit += @"var CKEditor_Controls=[],CKEditor_Init=[];function CKEditor_TextBoxEncode(d,e){var f;if(typeof CKEDITOR=='undefined'||typeof CKEDITOR.instances[d]=='undefined'){f=document.getElementById(d);if(f)f.value=f.value.replace(/</g,'&lt;').replace(/>/g,'&gt;');}else{var g=CKEDITOR.instances[d];if(e&&(typeof Page_BlockSubmit=='undefined'||!Page_BlockSubmit)){g.destroy();f=document.getElementById(d);if(f)f.style.visibility='hidden';}else g.updateElement();}};(function(){if(typeof CKEDITOR!='undefined'){var d=document.getElementById('#ClientID#');if(d)d.style.visibility='hidden';}var e=function(){var f=CKEditor_Controls,g=CKEditor_Init,h=window.pageLoad,i=function(){for(var j=f.length;j--;){var k=document.getElementById(f[j]);if(k&&k.value&&(k.value.indexOf('<')==-1||k.value.indexOf('>')==-1))k.value=k.value.replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&');}if(typeof CKEDITOR!='undefined')for(var j=0;j<g.length;j++)g[j].call(this);};window.pageLoad=function(j,k){if(k.get_isPartialLoad())setTimeout(i,0);if(h&&typeof h=='function')h.call(this,j,k);};if(typeof Page_ClientValidate=='function'&&typeof CKEDITOR!='undefined')Page_ClientValidate=CKEDITOR.tools.override(Page_ClientValidate,function(j){return function(){for(var k in CKEDITOR.instances){if(document.getElementById(k))CKEDITOR.instances[k].updateElement();}return j.apply(this,arguments);};});setTimeout(i,0);};if(typeof Sys!='undefined'&&typeof Sys.Application!='undefined')Sys.Application.add_load(e);if(window.addEventListener)window.addEventListener('load',e,false);else if(window.attachEvent)window.attachEvent('onload',e);})();";
			scriptInit = scriptInit.Replace("#ClientID#", ClientID);
			RegisterStartupScript(GetType(), "CKEditorForNet", scriptInit, true);
			RegisterStartupScript(GetType(), ClientID + @"_addControl", string.Format(@"CKEditor_Controls.push('{0}');
", ClientID), true);
			string script = string.Empty;
			if (config.CKEditorEventHandler != null)
				foreach (object[] item in config.CKEditorEventHandler)
				{
					script += string.Format(@"CKEditor_Init.push(function(){{CKEDITOR.on('{0}',{1});}});
", item[0], item[1]);
				}
			if (config.protectedSource != null && config.protectedSource.Length > 0)
			{
				string proSour = string.Empty;
				foreach (string item in config.protectedSource)
					proSour += @"
...
", ClientID, prepareJSON(), proSour, doPostBackScript);
			}
			else
...
", ClientID, prepareJSON(), doPostBackScript);
			...
			RegisterOnSubmitStatement(GetType(), "aspintegrator_Postback" + ClientID, string.Format("CKEditor_TextBoxEncode('{0}', {1}); ", ClientID, isInUpdatePanel ? 1 : 0));
			RegisterStartupScript(GetType(), "aspintegratorInitial_" + ClientID, script, true);
		...
		protected override void OnLoad(EventArgs e)
		{
			base.OnLoad(e);
			if (ViewState["CKEditorConfig"] != null)
				config = (CKEditorConfig)ViewState["CKEditorConfig"];
			else
				config = new CKEditorConfig(BasePath.StartsWith("~") ? ResolveUrl(BasePath) : BasePath);
		}
		...
		{
			return JSONSerializer.ToJavaScriptObjectNotation(config);
		}
	...
	public class CKEditorControlDesigner : System.Web.UI.Design.ControlDesigner
	{
		...
		{
            CustomEditor control = (CustomEditor)Component;
